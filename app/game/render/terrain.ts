...
export function createTerrain(
  ...
  // The smooth in-grid elevation field simply adds to the base height —
  // everywhere, so the buildable hills flow past the grid edge into the
  // countryside (whose decorative hills ramp in beyond FLAT_RADIUS on top).
  // The field is water-faded at the source, so the carve region is untouched.
  const heightAt = (x: number, z: number) => baseHeightAt(x, z) + elevation.heightAt(x, z);
  ...
  // field adds after the dilation so a nearby rise can't read as "land to
  // carve toward".
  ...
  const displacedAt = (x: number, z: number) => baseDisplacedAt(x, z) + elevation.heightAt(x, z);
