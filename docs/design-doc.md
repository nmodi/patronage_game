...
**Formation**: a plaza is a connected union of fully-open **4×4 core blocks** (open = bare buildable land, player paving, or land road cells — a generous crossing becomes a piazza the way real ones did; bridges never join). **Plaza furniture is plaza ground** *(August 2026)*: road-overlay buildings (market stall, plinth) and `paved` decorations (fountain, obelisk, colonnade) count as open *and* as paving, so a fountain centred in a paved court sits inside the piazza instead of punching a hole in it, and dropping a stall or plinth onto an authored court never breaks a core or unforms it (furniture footprints stay out of the walkers' network — nobody wades through the fountain). The 4×4 core is the minimum size *and* the strip-proofing — no 3-wide avenue ever contains one — and unions make any shape legal. A block qualifies **authored** (all paving) or **organic** (mean field ≥ `GATHER_FORM` *and* **enclosed**: every outward lane hits a tile, water, or the map edge within `PLAZA_ENCLOSE_REACH` (3) cells — a campo is leftover space framed by the city, never open meadow). Un-forming happens only when the player builds over the ground itself.
