...
**Formation**: a plaza is a connected union of fully-open **core blocks** (open = bare buildable land, player paving, or land road cells — a generous crossing becomes a piazza the way real ones did; bridges never join). **Plaza furniture is plaza ground** *(August 2026)*: road-overlay buildings (market stall, plinth) and `paved` decorations (fountain, obelisk, colonnade) count as open *and* as paving, so a fountain centred in a paved court sits inside the piazza instead of punching a hole in it, and dropping a stall or plinth onto an authored court never breaks a core or unforms it (furniture footprints stay out of the walkers' network — nobody wades through the fountain). The core is the minimum size *and* the strip-proofing — no 3-wide avenue ever contains one — and unions make any shape legal. An **authored** block is `PLAZA_CORE`² (4×4) of paving; an **organic** block is open ground with mean field ≥ `GATHER_FORM` *and* **enclosed** per one of the two `ORGANIC_TIERS` — an outward lane counts bounded when it hits a tile or water within `PLAZA_ENCLOSE_REACH` (3) cells (the map edge frames nothing — two cottages in a world corner are not a courtyard; a canal-side campo is), and each tier allows its own open lanes: a **small campo** (4×4, zero open lanes — a true courtyard, perfectly framed) or a **grand campo** (5×5, up to 2 open lanes — mass forgives a modest opening). A formed region must also be **public**: it contains or touches road-type ground (streets, paving, bridges) — a court fully walled by buildings is a private yard and never forms, so a small campo's frame always includes street frontage (the village green beside the road). The strictness split scales formation with city character with no population input: a village court framed flush forms a small campo, while a mature city's incidental pockets essentially never fully enclose (0 of 1,917 hot 4×4 blocks in the maxed demo), so sprawl isn't littered with minis — and since building only ever bounds more lanes and raises the field, **growth can never un-form a campo** (only building on its cells or razing its frame can). An organic region then **fills out**: it floods its open pocket (adjacent plaza-able ground except streets, up to `PLAZA_ENCLOSE_REACH` steps from the core), so a formed grand campo hugs its court walls instead of stranding a core-sized square (a small campo is fully walled — nothing to fill). *(August 2026 tuning, driven by the maxed demo city: unanimous enclosure as the only rule vetoed every hot pocket — no organic plaza could form in practice — while a 4×4 core with forgiveness found 13; the field bar is no lever at all (every framed pocket runs far past `GATHER_FORM`, raising it 3× changed nothing), so rarity lives in pocket geometry — the grand tier forms 4 there, each filled out to 43–76 cells, and the small tier fires only where a court is deliberately walled.)* Un-forming happens only when the player builds over the ground itself.
...
**Map overlays** *(built — August 2026, first view)*: a Civ-style overlay system — session-only `activeOverlay` state, one HUD toggle + legend (`ui/OverlayToggle.tsx`), one draped unlit mesh (`render/gatheringOverlay.ts`). View 1 is **Gathering**: the field as **four quantized heat bands** (`HEAT_BANDS`, multiples of `GATHER_FORM` at 0.5/1/2.5/6 — a mature city runs the field to ~13× the formable level, so the upper bands spread across that range; only the palest band is sub-formable, making the first color step the actionable edge — a continuous alpha wash was tried first and saturated flat in any built-up quarter), formed plazas filled and bordered gold (organic) or slate (authored), and the premade hub plazas (Town Center Plaza; old saves' Plaza/Small Plaza) highlighted in the authored slate — so the player watches a spot warm up before the city claims it. The roadmap's isochrone overlay is the intended second rider on this plumbing.

**Display in formed plazas**: the **Plinth** decoration (1×1, street furniture via `placesOnRoads` like the stall — sits on roads and plaza paving without severing either, and counts as plaza ground per Formation above) carries one plinth display slot, so a formed piazza shows sculpture through the ordinary Phase 9 display flow — no plaza-keyed persistence needed (plazas stay fully derived; the work keys to the plinth's tile like any host). It supersedes the 3×3 **Sculpture Display**, retired from the palette (defs remain; placed ones keep working). The plinth itself generates nothing and doesn't warm the gathering field — deliberate, so empty-plinth spam isn't a field-farming move; "displayed art warms the field" is a noted future extension that would relax the field's tiles-only contract.
