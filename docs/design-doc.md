...
**Freeform (any-angle) roads** *(built — July 2026)*: paved road tools (Path/Road/Avenue/Dirt Path/Stone Bridge) now place **at any angle**, not just the 8 octants — click a start node, move to preview a straight ribbon, click to commit and chain the next segment (Escape ends the chain). Endpoints snap to cell centers by default (roads meet buildings/other roads predictably) with the angle left free; holding **Shift** frees the endpoints entirely. Built as **graph + rasterization** — the cheap middle ground between the octant grid and a full Cities-Skylines spline engine: roads are stored as float world-space **segments** (`app/game/roadSegment.ts`, persisted `map.roads` — the geometry/edit source of truth) and **rasterized into road cells** (`app/game/roadRaster.ts`, supercover with a guaranteed-8-connected cell set — asserted in `roadRaster.check.ts`) that the sim reads via `deriveSimTiles`. Because connectivity, foot-traffic, citizen walks, and water/collision all read `type:"road"` cells, **none of them changed** — the derived cells (`derived:true`, never persisted, never razed individually, never drawn by the cell renderer) union under the canonical `tiles` so a real building always wins its cell. A new isolated ribbon renderer (`app/game/render/roadRibbonRenderer.ts`, one merged `VertexData` per material — paved/dirt/bridge — with square end-caps for butt joints and vertical parapet rails on bridges) draws the true-angle geometry; the ghost is the same ribbon. Cost is per rasterized cell (reusing `planLinearPlacement`), and raze hit-tests the nearest segment and removes it atomically (refunding salvage). Save v8 is a *preserving* migration (absent `roads` → `[]`, like v6's `mapSeed`), so **legacy grid/diagonal roads keep working** on the existing cell renderer and coexist with freeform ones. Deferred: mitered joints, bezier curves, width transitions, segment-angle-aware `roadSnap`, legacy-cell→segment conversion. The octant grid drag (Diagonal streets, above) still backs the linear *decorations* (fence/wall/colonnade), which stay cardinal.
