...
Dev helpers: `/?demo` seeds a visual test city, `&pause` freezes the tick for stable screenshots, `&map=<seed>` forces a specific map (water archetype / river course / coast **and terrain terraces**) for iteration — works with `?demo` too, `&cam=x,z[,radius[,alpha[,beta]]]` frames a world position for headless screenshots.

---

## Terrain Elevation (Terraces) *(built — August 2026)*

Maps carry **quantized terrace levels** (integer 0–2 per cell, 0.3 wu per step — never continuous slopes), derived from the persisted `elevationSeed` exactly like water derives from `mapSeed` (`app/game/map/elevation.ts`, memoized `getElevation`, own `elevation:` rng namespace so no other seeded pick reshuffles; verified by `elevation.check.ts`). ~40% of maps roll the classic flat plain; hilly ones quantize two low-frequency sine octaves into plateaus tens of cells across (~80% of cells stay level 0, so the city core always has plains). Two invariants everything leans on: **4-neighbor cells never differ by more than one level** (a lowering pass enforces it), and land within 2.5 wu of water is level 0 with higher levels set back further — banks, bridges, and the whole water render pipeline never meet a cliff. Gameplay: **buildings need a footprint on a single level** (checked in `planPlacement`/`canPlaceAt`, so ghost and planner can't drift); **roads, linear decorations, and walkers cross any one-level step freely** — elevation blocks placement like water does but never touches output or connectivity (principle 6; the BFS stays adjacency-only). Save v13 mirrors v6: pre-elevation saves get `elevationSeed: null` — forever flat, since newly rolled hills would strand buildings across cliffs; `?demo` also runs flat so the hand-placed layout keeps placing. Render: a terrace overlay mesh (`render/terrain.ts`) draws flat per-cell tops with tufo cliff walls at each step over the untouched flat plain, the countryside beyond the grid continues each rim cell's height before the decorative hills add on, and everything standing on the ground samples one shared helper (`render/groundLevel.ts`, the `bridgeLiftAt` precedent): building batches, aprons, display art, markers, ghost/previews, junction pads, dirt-path chunks (vertex-draped), and citizens (who lerp between cell levels so a step crossing matches the road ramp). Cardinal road cells whose road neighbor sits one level up/down render as chorded ramps reusing the bridge-deck transform; pointer picking tests one plane per level top-down (`render/placement.ts`), accepting a hit only where the cell really is that level.
