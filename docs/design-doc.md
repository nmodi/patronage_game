...
**Ambient bed** *(built — August 2026)*: a single looping crowd-murmur bed that makes the city sound as busy as it looks. Loudness is derived, never a new sim input: `ambienceGain(population, radius)` in `app/game/audio/ambience.ts` = `bustle(population)` (the same citywide 0..1 crowd factor foot traffic uses — an empty city is silent by construction) × a camera-closeness factor (linear over the camera's radius clamp 3–80, flooring at `ZOOM_QUIET` 0.3 so zooming out quiets but never kills the city). The engine (`useAmbience.ts`) mirrors `useMusic`'s module-singleton HTMLAudio shape — menu-click gesture start, pointerdown retry for `?demo`, direct volume sets with no ramp (population steps are tiny, zoom is per-frame continuous) — and the render loop feeds it `camera.radius` each frame through a no-op-unless-changed setter. Still no `AudioContext`: the loop seam is a crossfade baked into the asset (`scripts/make-ambience-loop.py`, stdlib-only slice + equal-power tail-into-head fold) from Metzik's Visby medieval-market field recording (CC BY 4.0, `CREDITS.md`). `ambienceVolume` in the store (persisted, own Settings slider, 0 = mute); `ambience.check.ts` pins the gain curve's endpoints and monotonicity.
