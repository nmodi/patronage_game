...
**Freeform plazas** *(built — August 2026)*: player-painted piazzas beside the prefab stamps. **Plaza Paving** (Roads tab, 12ƒ/cell, florins only, no inspiration) is a rect-dragged ground surface — anchor click, hover the opposite corner, click to commit, cardinal-only, capped at 24 cells per axis per drag (`buildRectStretch` in `roadStretch.ts`). Unlike road drags, blocked cells (water, any occupied tile) are **skipped uncharged** (`planAreaPlacement` in `placementRules.ts`, routed by `metadata.areaDrag`) so one rect floods around obstacles and joins existing paving free — the ghost previews only the cells that will land. Cells persist as ordinary per-cell road-family tiles (colonnade-style, no save change); `type: "road"` gives them connectivity conduction, citizen walkability, the worn-earth verge halo, market-stall placement, and raze-drag-sweep removal (6ƒ salvage) for free. Renders as directionless finished-slab limestone (`getPavingMaterial` in `render/paths.ts`, 4 position-hashed seed variants) at street height.

**Piazza hubs** — hub status is a **local openness check, never region area** (monotonic by construction: laying more paving only ever turns hubs on; strip-proof: no cell of a 1-wide run has a mostly-paved window; no flood fill). Two qualification routes, derived every compute inside `computePlazaConnectivity` and persisted nowhere:

- **Centerpiece route** — a **Fountain, Obelisk, or Sculpture Display** qualifies as a secondary hub when ≥60% of the 7×7-window cells around its footprint (claimed-cell bbox + 2, grid-clamped) are paved ground (`PLAZA_HUB_RING`/`PLAZA_HUB_PAVED_FRACTION`). "Paved ground" mirrors the render side's `pavedCells`: paved roads plus any `metadata.paved` building's claims — so a piazza's own furnishings count *for* its window, and the threshold slack means stalls and plinths never un-hub it.
- **Open-square route** — a bare campo qualifies through any paving cell whose own 7×7 window is ≥90% paved (`PLAZA_CAMPO_HALF`/`PLAZA_CAMPO_PAVED_FRACTION`, 4-cell furnishing slack): a genuine open square, priced above the fountain-piazza route by construction.

Qualified cells behave exactly like prefab plaza cells in the BFS — distance resets to 0 **when reached** from the Main Plaza network, so an isolated piazza radiates nothing. A qualified centerpiece stays a bonus *recipient* too (strength 1 from its own cells), so its inspiration keeps scaling. Feedback: centerpiece decorations carry a hover tooltip row — "Piazza hub ✓" or "Piazza hub: N/M paved cells" (`hubRingStatus`) — and paving cells show a name card (the one road-family surface exempt from the roads-are-noise hover skip). The Town Center Plaza remains the unique Main Plaza; the secondary Plaza/Small Plaza prefabs stay as one-click stamps (palette retirement is a roadmap decision after play).

*(An earlier draft specified automatic cluster/hub road generation. Cut — manual placement fits the cozy hands-on scale. A second rejected draft derived freeform hub strength from painted region area — rejected because every anti-strip-gaming shape rule either breaks monotonicity or reintroduces hard radii; the openness check above is the design that survives.)*
...
### Civic / Landmark
- **Plaza** / **Small Plaza** / **Town Center Plaza** *(built)* — generates Inspiration, displays Works. The Town Center Plaza is the **Main Plaza** — the connectivity hub; Plazas and Small Plazas (a 5-cell piazzetta, chapel-width) are secondary hubs that refresh its reach *(since August 2026, freeform painted piazzas — Plaza Paving + a qualifying centerpiece or open square — are the second way to make a secondary hub; see Roads → Freeform plazas)*
...
### Decorative
- Tree *(built)*, Cypress *(built — stretched/sunk Kenney pine)*, Bell Tower / Campanile *(built — the cathedral's old tower as a freestanding decoration; July 2026: a secondary connectivity hub with an inspiration trickle)*, Fountain *(built; August 2026: piazza centerpiece + a **fountain display slot** — a sculptor work crowns the basin, no pedestal, `FOUNTAIN_CROWN_Y` seat)*, Vineyard *(built — dirt furrows planted with rows of vine-on-post trees)*, Olive Grove *(built)*, Colonnade *(built)*, Memorial Column / Obelisk *(built — kitbashed pillar + block + point roof)*, Bush *(built — nature-kit plant variants)*, Rocks + Boulder *(built — nature-kit rocks, limestone tint)*, Wooden Fence *(built — nature-kit rail/plank segments)*, Low Stone Wall *(built — wall-block slab kitbash with end posts)*, Sculpture Display *(built — a placeable stone plinth; displays a sculpture, Phase 9)*, Garden, Gallery Wall, Tower House (casa-torre — slim San Gimignano-style family tower, a skyline element; small inspiration/prestige boost). *(The Loggia left this list twice over — it landed as a civic blueprint-commission structure in July 2026, then was removed from the game in Aug 2026; its design is shelved in [building-effects.md](building-effects.md) → Blueprint lane.)*
