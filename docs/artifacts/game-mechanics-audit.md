...
| **Hubs / conductors** | Hub set = plazas + `isHub` buildings (bell tower), derived from metadata via `PLAZA_IDS`; road-cost conductors via `ROAD_OVERLAY_IDS` (e.g. market stall); plus derived **piazza hub cells** (next row). | `connectivity.ts` |
| **Freeform piazza hubs** *(Aug 2026)* | A local openness check, never region area (monotonic, strip-proof, no flood fill). Centerpiece route: fountain/obelisk/sculpture_display qualifies when ≥`PLAZA_HUB_PAVED_FRACTION` (0.6) of the 7×7 window around its claimed-cell bbox (+`PLAZA_HUB_RING` 2, grid-clamped) is paved ground (paved roads + `metadata.paved` claims). Open-square route: any `plaza_paving` cell whose 7×7 window is ≥`PLAZA_CAMPO_PAVED_FRACTION` (0.9) paved. Qualified cells reset the BFS to 0 when reached; derived per compute, persisted nowhere. Tooltip readout via `hubRingStatus()`. | `connectivity.ts` → `computeHubCells()`, `hubRingStatus()`; knobs in `constants.ts` |
...
| Mechanic | What it does | Code |
|---|---|---|
| **Display slots** | Buildings/plazas carry typed slots (`painting`/`statue` interior, `plinth` exterior with a footprint cell, `fountain` cell-less — a sculptor work crowns the fountain basin, Aug 2026). Painters fill painting; sculptors fill statue+plinth+fountain; architects none. | `app/game/art/display.ts` → `slotAccepts`, `SLOT_KINDS_BY_ARTIST`, `DisplaySlotDef` in `types.ts`; crown render in `mapRenderer.ts` `buildDisplayArt` (`FOUNTAIN_CROWN_Y`) |
...
| **Effect flags** | `isHub` (plazas + bell_tower), `placesOnRoads` (market_stall), `footTraffic` (market_stall), `connectionBonus` (stall 1.0), `paved`, `linear` (colonnade/fence/stone_wall), `areaDrag` (plaza_paving — rect-drag, skip-blocked-cells placement), `prestigeOnBuild` (cathedral 25), `roadWidth` (5 road variants), `buildCost` (landmarks; also future blueprint structures), `commissionOnly` (none currently — the blueprint roster is empty), `materialStorage` (warehouse 50). `costEscalates` is computed from `type`, not a field. | `buildings.ts` |
...
| **Plaza Paving** *(Aug 2026)* | Rect-dragged road-family ground surface (12ƒ/cell, no inspiration): anchor click → opposite corner → commit, cardinal-only, 24-cell axis cap. Blocked cells (water, occupied) skip uncharged — one rect floods around obstacles (`planAreaPlacement`); ghost previews only the landing cells. Free-riders from `type: "road"`: connectivity conduction, walkability, verge halo, stall placement, raze-sweep (6ƒ salvage). Renders as directionless slab limestone, 4 position-hashed variants. | `placement/roadStretch.ts` → `buildRectStretch()`; `placement/placementRules.ts` → `planAreaPlacement()`; `render/paths.ts` → `getPavingMaterial()`; `render/roadRenderer.ts` paving batches |
...
- Freeform plazas (Aug 2026): rect-dragged Plaza Paving surface, centerpiece/open-square piazza-hub qualification in connectivity, fountain display slot (sculptor work crowns the basin). Leftovers (render polish, prefab-retirement decision, ghost live-count, site-bound fountain commissions) in roadmap.md.
