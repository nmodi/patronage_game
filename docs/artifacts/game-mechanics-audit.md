...
| **Freeform plaza formation** | A plaza = connected union of fully-open core blocks (open = bare buildable land, plaza paving, land road cells — bridges never — plus plaza furniture: road-overlay buildings and `paved` decorations, which also count as *paving*, so a fountain/stall/plinth in a paved court joins the plaza rather than breaking it). **Authored** blocks are `PLAZA_CORE`² (4×4) all paving; **organic** blocks follow `ORGANIC_TIERS` — small campo 4×4 with every outward lane bounded, or grand campo 5×5 with ≤2 open lanes — where a lane is bounded when it hits a tile or water within `PLAZA_ENCLOSE_REACH` 3 (the map edge frames nothing), plus mean field ≥ `GATHER_FORM` 14. A formed region must be **public** (contains/touches road-type ground — walled yards never form), and an organic region **fills out** its open pocket (able-non-street ground ≤ 3 steps from the core). Growth can never un-form a campo (building only bounds more lanes, raises the field, adds access). Region organic if any block is field-qualified — paving more never demotes. Fully derived, nothing persisted; `anchor` = row-major-first core cell (naming's future key). | `gathering.ts` → `FormedPlaza`, `plazaCells` |
...
- Graphics G1–G4 + generated kit pieces + category-identity pass; G5 mostly: river + bridge, decorative citizens, obelisk, seeded water archetypes, diagonal streets, snap-to-road + 45° buildings, market stall + foot traffic, main menu. Freeform plazas (gathering field, core-block formation — authored 4×4 paving / organic two-tier small-and-strict or grand-and-forgiving with street access + pocket fill-out — formed-plaza hubs + inspiration, furniture-as-plaza-ground + the 1×1 Plinth display pedestal — Sculpture Display retired) + the map-overlay system's first view (August 2026) — premade Plaza/Small Plaza retired from the palette.
