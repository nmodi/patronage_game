...
Four pieces are built in code (`render/proceduralPieces.ts`) instead of loaded: **`proc:block`**, **`proc:roof-gable`**, **`proc:gable-end`**, **`proc:roof-hip`**. They enter through the ordinary file path — `getContainer` branches on the `proc:` prefix and everything downstream (material conversion, `MATERIAL_TINTS`, tints, desaturation, batching, blend stretch) treats them as untextured Nature-Kit-style files. Each fills the envelope of the kit piece it replaces, because the manifest's positions and `fit` values are tuned to those bounds — exactly for the block, and *core*-only for the roof, whose tiles stand proud of it (`ROOF_TILE_BULGE`). `proceduralPieces.check.ts` enforces both.

**For what's still open** — the panel-to-fitting migration, the artist commission ([artist-brief.md](artist-brief.md)) — see [procedural-pieces.md](procedural-pieces.md). This section is the field notes; that one is the plan.
...
- **The surviving corner bars flicker**, and the panels are why: a panel is a full-face slab carrying a quoin at *each* of its own corners, so at a building corner two perpendicular panels each claim the same ~0.1×0.1 wedge of space and z-fight over it. Removing the block's quoins ended the doubling but not this — the two fighting surfaces are both panels. The fix is replacing the panels with loose fittings on plain stucco — commissioned surrounds plus the extracted leaves, the pattern the residences already use (see procedural-pieces.md §1) — which also finally retires `make-mint-quoins.py`; nothing about the block or the tint can reach it.
