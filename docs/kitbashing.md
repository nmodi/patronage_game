...
- **The kit's door/window/arch panels were full-face slabs** carrying the atlas's baked quoins — one salmon bar per panelled corner, z-fighting where two perpendicular panels claimed the same wedge. Retired (July 2026): every opening across the roster is now a generated fitting (`windowOn`/`doorOn`/`archWindow`/`oculusOn`/`portalOn` in the manifest), which ended the corner-quoin flicker for good. See procedural-pieces.md §1.
