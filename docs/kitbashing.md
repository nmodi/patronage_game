...
- **The surviving corner bars flicker**, and the panels are why: a panel is a full-face slab carrying a quoin at *each* of its own corners, so at a building corner two perpendicular panels each claim the same ~0.1×0.1 wedge of space and z-fight over it. Removing the block's quoins ended the doubling but not this — the two fighting surfaces are both panels. The fix is replacing the panels with loose fittings on plain stucco — generated surrounds plus the extracted leaves, the pattern the residences and palazzo now use (see procedural-pieces.md §1) — which also finally retires `make-mint-quoins.py`; nothing about the block or the tint can reach it. 53 panel refs remain (workshops, bakery, tavern, chapel, cathedral portals, round windows).
