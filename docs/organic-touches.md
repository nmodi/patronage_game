...
**2. Roofline variation from density.** *(partly built — Rialto roofscape, August 2026: every house now hashes between four whole roof forms, gable or hip, each carrying one or two chimney pots. That is the variation without the density read — it is seeded per cell, not earned by neighbours.)* What is left is the density half: in a packed housing cluster roofs grow dormers, drying racks, a rooftop terrace; isolated cottages stay clean-roofed. → Neighbor count within 1–2 cells, seeded per cell — feeding `roofForms` a density-weighted pick is the cheap version, since the form machinery already exists. → Kitbash roof pieces and the ridge-along-X convention are documented in `render/CLAUDE.md`. *This is what makes an old town read as layered rather than tiled.*
