...
**Parallel track — Audio (music & SFX)** *(added July 2026; music and interaction-SFX slices shipped)*: touches no sim systems, so it runs alongside the tiers rather than inside them. Remaining: the ambient bed + Mid/Late-era tracks — see the backlog entry.
...
- **Ambience** *(the music and interaction-SFX slices shipped — see the design doc's Music & Sound section)* — an ambient bed: birdsong/wind at the edges, light crowd murmur near plazas (could scale with the existing `bustle` number for free). Looping/crossfading beds are the one place the shipped HTMLAudio-clone SFX engine may want a shared AudioContext. Nothing needs to be spatial at this camera distance — no Babylon sound-engine dependency unless positional audio earns its way in later. Also pending: Mid/Late-era tracks for the shipped era system (`TRACKS` in `app/game/audio/music.ts` — append and done).
