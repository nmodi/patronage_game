...
- **Real citizen agents — a principle-1 reversal candidate** *(added August 2026)* — a pattern worth naming: feature ideas keep wanting per-spot crowd granularity and keep getting derived proxies instead — market-stall foot traffic (catchment in `city/traffic.ts`; reading the walkers was rejected), zoomed-in ambience (the render-side bustle field, render→audio only), organic plaza formation (the gathering field). Each proxy is honest and deterministic, but if ideas keep arriving that want real crowds, promoting the decorative walkers to sim agents should be decided *deliberately* — the way the materials pools deliberately reversed "never a stockpile" — not backed into one coupling at a time. The recorded blockers (design doc, Ambient bed): walkers are unseeded, render-side, and freeze on pause, so sim consumption means seeding them and driving them from the tick (determinism work that touches saves/replay); and naive density coupling dilutes as the walk network grows — a new road must never lower any building's output (principle 6). Until this is decided, the standing middle path holds: proxies stay canonical, walkers are steered to visualize them.
- **Plaza naming** *(added August 2026 — the deferred half of the freeform-plazas design)* — formed plazas get names at formation: context-derived (nearest landmark, the anchoring artwork or its patron family), player-renamable, Italian proper nouns welcome ("Piazza della Lana"). This is the system's first persistent state: a save-migration bump adding a per-plaza record `{anchorCell, name, customName?}` keyed to `FormedPlaza.anchor` (already deterministic for exactly this). Identity across grow/shrink/split/merge needs design. Follow-ups that ride the same record: the "campo petitions for a fountain" commission offer (touches `maybeOfferCommission` — rng-draw-order trap). *(Display in plazas shipped August 2026 without needing this record — the 1×1 Plinth decoration hosts the slot; see the design doc.)*
...
2. **Isochrone overlay** — the 0-1 BFS from plazas already exists; render it as travel-time contour bands. Isochrones are a literal Mapbox API product, and it's a genuinely useful overlay for placing the next plaza. Days, not weeks. Fits design constraints: derived visualization, not citizen pathfinding. *(The overlay plumbing shipped August 2026 with the gathering view — `activeOverlay` in the store, `render/gatheringOverlay.ts`, `ui/OverlayToggle.tsx`; this becomes view #2 on it.)*
