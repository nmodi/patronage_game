...
- **Music & sound effects** *(added July 2026 — the game is currently silent)* — its own parallel audio track, graphics-track style. **Slice 1, ambience + feedback**: an ambient bed (birdsong/wind at the edges, light crowd murmur near plazas — could scale with the existing `bustle` number for free) plus interaction SFX (placement thunk, raze, coin chime on commission payout, the offer-arrival card's bell). **Slice 2, music**: a small looping period bed (lute/recorder, CC0/CC-BY sources to match the Kenney-asset approach) with volume/mute in Settings. Nothing needs to be spatial at this camera distance, so plain WebAudio from the React layer likely suffices — no Babylon sound-engine dependency unless positional audio earns its way in later.
