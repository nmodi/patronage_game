...
| `proc:louvres` | louvred shutter pair (August 2026) — two leaves (slab + stiles/rails + 7 recessed slats), green `wood`, lapping 0.002 onto the surround all round. `~1` is the OPEN twin, the same leaves swung 142° about a hinge plane that stays at local x = 0 in both states so one manifest position serves either; that swing is the physical ceiling, not a taste call — sideways reach is `w(1 − cos swing) + t·sin swing`, and folding flat wants a full window-width of bare wall beside each opening where the townhouse's 0.25 column pitch leaves 0.12. The one piece deliberately **not** x-centered (the open twin) | every house window (`houseWindow` → `louvresOn`), `buried` so the leaves overhang the footprint instead of shrinking the house. Which state a given window gets is the one thing in the city that is **not** position-hashed — `SHUTTER_SEED` in `assetLibrary.ts` is fresh per page load |
